...
impl Query {
    ...
    async fn generate_code_or_text(
        ...
        match user {
            Some(user) => {
                ...
                let string_key = format!("chats:{}:{}", name, email);
                let generated_code_or_text: String = utils::gpt::generate_code_or_text(
                    code_or_text_generation_input.prompt.to_string(),
                )
                .await?;
    ...
    async fn get_code_or_text_generated_by_user(
        ...
        let db = ctx.data::<RocksDB>()?;
        let string_key = format!("chats:{}", &user);
...
// withiout threads and its blocking
// #[tokio::main]
// async fn main() {
//     dotenv().ok();
